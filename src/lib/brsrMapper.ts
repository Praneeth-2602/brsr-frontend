...
function buildBaseRow(data: any) {
  ...
  row["11. Paid-up Capital (INR)"] = entity.paid_up_capital;
  row["12. Contact Person Details"] = entity.contact_person_details;
  row["13. Reporting boundary (Standalone/Consolidated/Both)"] = entity.reporting_boundary;
  row["14. Name of assurance provider"] = entity.assurance_provider;
  row["15. Type of assurance (Reasonable/Limited)"] = entity.assurance_type;
  ...
  row["18.a National Plants (Nos.)"] = locations.national_plants;
  row["18.b National Offices (Nos.)"] = locations.national_offices;
  row["18.c International Plants (Nos.)"] = locations.international_plants;
  row["18.d International Offices (Nos.)"] = locations.international_offices;

  row["19.a International Countries (Nos.)"] = markets.international_countries;
  ...
  row["20.A Total Permanent Employees (Nos.)"] = emp.total_permanent;
  row["20.A Permanent Male Employees (Nos.)"] = emp.permanent_male;
  row["20.A Permanent Female Employees (Nos.)"] = emp.permanent_female;
  row["20.A Other than Permanent (Nos.)"] = emp.other_than_permanent;
  row["20.A Other Male (Nos.)"] = emp.other_than_permanent_male;
  row["20.A Other Female (Nos.)"] = emp.other_than_permanent_female;
  row["20.A Total Employees (Nos.)"] = emp.total_employees;
  row["20.A Total Male (Nos.)"] = emp.total_male;
  row["20.A Total Female (Nos.)"] = emp.total_female;

  const workers = employees.workers || {};
  row["20.B Permanent Workers (Nos.)"] = workers.total_permanent;
  row["20.B Permanent Male Workers (Nos.)"] = workers.permanent_male;
  row["20.B Permanent Female Workers (Nos.)"] = workers.permanent_female;
  row["20.B Other Workers (Nos.)"] = workers.other_than_permanent;
  row["20.B Other Male Workers (Nos.)"] = workers.other_than_permanent_male;
  row["20.B Other Female Workers (Nos.)"] = workers.other_than_permanent_female;
  row["20.B Total Workers (Nos.)"] = workers.total_workers;
  row["20.B Total Male Workers (Nos.)"] = workers.total_male;
  row["20.B Total Female Workers (Nos.)"] = workers.total_female;

  const da_emp = employees.differently_abled_employees || {};
  row["20.C DA Employees Total Permanent (Nos.)"] = da_emp.total_permanent;
  row["20.C DA Permanent Male (Nos.)"] = da_emp.permanent_male;
  row["20.C DA Permanent Female (Nos.)"] = da_emp.permanent_female;
  row["20.C DA Other (Nos.)"] = da_emp.other_than_permanent;
  row["20.C DA Other Male (Nos.)"] = da_emp.other_than_permanent_male;
  row["20.C DA Other Female (Nos.)"] = da_emp.other_than_permanent_female;
  row["20.C DA Total Employees (Nos.)"] = da_emp.total_employees;

  row["21. Women Representation"] = "";
  row["21.a Board Total (Nos.)"] = women.board_of_directors_total;
  row["21.b Board Women (Nos.)"] = women.board_of_directors_women;
  row["21.c KMP Total (Nos.)"] = women.kmp_total;
  row["21.d KMP Women (Nos.)"] = women.kmp_women;
  ...
  row["23.a Group Entity"] = first_holding.name;
  const raw_type = first_holding.type;
  // row["23. Group Entity Type"] = raw_type;
  row["23.b Group Entity Type"] = mapGroupEntityType(raw_type);
  row["23.c % Shares (%)"] = first_holding.percent_shares_held;

  row["24.a CSR Applicable (Yes/No)"] = csr.is_applicable;
  row["24.b CSR Turnover (INR Cr)"] = csr.turnover_inr_cr;
  row["24.c CSR Net Worth (INR Cr)"] = csr.net_worth_inr_cr;

  row["25. Grievance Redressal"] = "";
  const mech = (grievances.mechanism_in_place || {});
  row["25.a Communities (Yes/No)"] = mech.communities;
  row["25.a Investors (other than shareholders) (Yes/No)"] = mech.investors_other_than_shareholders;
  row["25.a Shareholders (Yes/No)"] = mech.shareholders;
  row["25.a Employees and workers (Yes/No)"] = mech.employees_and_workers;
  row["25.a Customers (Yes/No)"] = mech.customers;
  row["25.a Value Chain Partners (Yes/No)"] = mech.value_chain_partners;
  row["25.a Others (Yes/No)"] = mech.other_please_specify;

  const filed = (grievances.filed || {});
  row["25.b Communities (Nos.)"] = filed.communities;
  row["25.b Investors (other than shareholders) (Nos.)"] = filed.investors_other_than_shareholders;
  row["25.b Shareholders (Nos.)"] = filed.shareholders;
  row["25.b Employees and workers (Nos.)"] = filed.employees_and_workers;
  row["25.b Customers (Nos.)"] = filed.customers;
  row["25.b Value Chain Partners (Nos.)"] = filed.value_chain_partners;
  row["25.b Others (Nos.)"] = filed.other_please_specify;

  const pending = (grievances.pending || {});
  row["25.c Communities (Nos.)"] = pending.communities;
  row["25.c Investors (other than shareholders) (Nos.)"] = pending.investors_other_than_shareholders;
  row["25.c Shareholders (Nos.)"] = pending.shareholders;
  row["25.c Employees and workers (Nos.)"] = pending.employees_and_workers;
  row["25.c Customers (Nos.)"] = pending.customers;
  row["25.c Value Chain Partners (Nos.)"] = pending.value_chain_partners;
  row["25.c Others (Nos.)"] = pending.other_please_specify;
  ...
  for (const category of ["environment", "social", "governance"]) {
    for (const item of itemsForCategory(risks, category)) {
      riskRows.push({
        ...
        "26. Financial Impact (Yes/No)": item.financial_implications,
        "26. Approach to Adapt/Mitigate (Yes/No)": item.approach_to_adapt_mitigate,
      });
    }
  }
